...
if [ "$1" = "dev" ]; then
  ...
  version=$(git describe --abbrev=0 --tags 2>/dev/null || echo "v0.0.0+0")
...
ValidateReleaseVersion() {
  ...
  if [[ ! "$releaseVersion" =~ ^v[0-9]+\.[0-9]+\.[0-9]+\+[0-9A-Za-z.-]+$ ]]; then
    echo "Error: release version must match vX.Y.Z+A, got: $releaseVersion"
    return 1
  fi
}
